...
local function rebuildStorageData()
    ...
    async:newUnsavableSimulationTimer(0.1, function()
        ...
        local dangerMode = storage.globalSection(globalStorage.storageName):get("itemDangerMode")
        globalStorage.data.itemsData = require("scripts.morrowind_world_randomizer.generator.items").generateData({smart = itemSafeMode, unsafe = dangerMode})
        step = step + 1
    end)
    async:newUnsavableSimulationTimer(0.1, function()
